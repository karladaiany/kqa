...
const DeployCard = () => {
	...
	const formatDeployText = () => {
      ...
      <p style="text-align: center; color: #9C27B0; font-family: Verdana;"><b>Gerar versão para deploy</b></p>`;
		...
		const branchPrincipal = fields.find(
			(f) => f.label === "Branch principal"
		);
		...
		for (
			...
			if (titulos[i] && fieldValues[titulos[i].id]) {
				blockContent += `<b>Título:</b> ${
					fieldValues[titulos[i].id]
				}<br>`;
			...
			if (blockContent) {
				...
				if (
					i <
					Math.max(titulos.length, links.length, prs.length) - 1
				) {
					plainText += "\n";
				}
			}
			...
			<div className="card-header">
				...
				<div className="card-filters">
					<button
						...
						onClick={addBranchAndPRFields}
					>
						+ Branch e PR
					</button>
					<button
						className="generate-all-btn"
						onClick={addFeatureFlagField}
					>
						+ Feature flag
					</button>
					<button
						className="generate-all-btn"
						onClick={addRunnerField}
					>
						+ Runner
					</button>
					<button
						className="generate-all-btn"
						onClick={addTitleLinkPRFields}
					>
						+ Título, Link e PR
					</button>
				</div>
