...
const FileGeneratorCard = ({ generatorFunctions }) => {
  ...
  const isButtonDisabled = () => {
    ...
    const invalidNumRecords =
      !numRecords || parseInt(numRecords) < 1 || parseInt(numRecords) > 1000;
  ...
  const generateSingleRecord = () => {
    ...
    if (selectedFields.numColaboradores)
      record.number_of_employees = generatorFunctions.generateNumEmployees();
    ...
    if (selectedFields.cpf)
      record.cpf = generatorFunctions.generateCPF().raw?.replace(/\D/g, '');
