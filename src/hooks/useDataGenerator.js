...
import {
	generateCPF as generateValidCPF,
	generateCNPJ as generateValidCNPJ,
	generateRG as generateValidRG,
...
import {
	eredeTestCards,
	getEredeTestCardByStatus,
	getEredeTestCardStatuses,
} from "../generators/eredeTestCards";
...
const cartoesBandeiras = {
	...
	erede: {
		credito: {
			// Type 'credito'/'debito' is not really applicable but structure expects it
...
const gerarNumeroCartao = (bandeira, tipo) => {
	...
	if (
		!bandeira ||
		!cartoesBandeiras[bandeira] ||
		!cartoesBandeiras[bandeira][tipo]
	) {
		// Se não especificar bandeira ou tipo, gera aleatório
		const todasBandeiras = Object.keys(cartoesBandeiras).filter(
			(b) => b !== "erede"
		); // Excluir erede do sorteio aleatório
		bandeira = faker.helpers.arrayElement(todasBandeiras);

		// Garantir que existe configuração para o tipo selecionado
		const tiposDisponiveis = Object.keys(cartoesBandeiras[bandeira]);
		tipo = faker.helpers.arrayElement(tiposDisponiveis);
	}

	const config = cartoesBandeiras[bandeira][tipo];

	// Verificar se o array de prefixos não está vazio
	if (!config.prefixos || config.prefixos.length === 0) {
		throw new Error(
			`Configuração inválida para bandeira ${bandeira} tipo ${tipo}: prefixos vazio`
		);
	}
...
export const useDataGenerator = () => {
	...
	const generatePerson = () => {
		...
		const telefone = `(${ddd}) 9${numeroBase.slice(
			0,
			4
		)}-${numeroBase.slice(4)}`;
		...
		return {
			...
			endereco: {
				...
				complemento:
					faker.helpers.arrayElement(["", "Apto", "Casa", "Sala"]) +
					" " +
	...
	const generateCreditCard = (bandeira = "", tipo = "", eredeStatus = "") => {
		...
		return {
			...
			tipo:
				tipo === "multiplo"
					? `${tipoReal} (múltiplo)`
					: tipoReal || "credito", // Mostra o tipo real com indicação de múltiplo
			isErede: false,
		};
	};
