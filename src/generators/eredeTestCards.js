...
export const eredeTestCards = [
  ...
  {
    status: "NEGADA_SALDO",
    description: "Negada - Saldo Insuficiente",
  ...
  },
  {
    ...
    description: "Negada - Erro Genérico",
  ...
  },
  {
    ...
    description: "Negada - Cartão Inválido/Bloqueado",
...
export const getEredeTestCardByStatus = (status) => {
  return eredeTestCards.find(card => card.status === status) || eredeTestCards[0]; // Fallback to the first card
};

// Helper to get all available statuses for the select dropdown
export const getEredeTestCardStatuses = () => {
  return eredeTestCards.map(card => ({ status: card.status, description: card.description }));
};
